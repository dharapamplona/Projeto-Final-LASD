...
/* ABAIXO AS RELAÇÕES UTILIZADAS PRA OS VALORES MOSTRADOS NO DISPLAY LCD 
...
tensão sensor de temperatura  = leitura_adc 
 ...
 /// CRIANDO A RELAÇÃO PARA O CALCULO DE PORCENTAGEM 
  
  (mAXIMO PWM)255 - 100(%)
   Dutyccle       - Porcentagem 
   
   Porcentagem = duty*0.3921568627
   
 /// Criando a função limitante para que mesmo que o valor máximo de temperatura seja ultrapassado a rotação do motor irá continuar no maximo
 
 80º equivale a 164 depois da conversão do adc portanto teremos uma relação do tipo:
		164      -    255
		leitura  -    duty 
		
		duty = leitura * 1.55487
